...
for i=1:number/2
    ...
    % Plot fit with data.
    getFitPlot(fitresult,xData,yData,fittingView,strcat("Hall fitting result of ",thicknessForTag(i)),temperatureForTag(i));
...
%     xWal = abs(dataRaw{i,2});
    ...
    % Plot fit with data.
    getFitPlot(fitresult,xData,yData,fittingView,strcat("WAL fitting result of ",thicknessForTag(i)),temperatureForTag(i));
...
%% ne vs thickness
getScatter(thicknessForNum(1:number/2),ne,generalView,"n_e vs Thickness",temperatureForTag(i))
xlabel {Thickness (nm)}
ylabel {n_e (cm^{-3})}

%% D vs thickness
getScatter(thicknessForNum(1:number/2),dif,generalView,"D vs thickness",temperatureForTag(i))
xlabel {Thickness (nm)}
ylabel {D (cm^2/s)}

%% L_SO vs thickness
getScatter(thicknessForNum(1:number/2),lso,generalView,"L_{SO} vs thickness",temperatureForTag(i));
xlabel {Thickness (nm)}
ylabel {L_{SO} (m)}

%% L_Phi vs thickness
getScatter(thicknessForNum(1:number/2),lphi,generalView,"L_{\phi} vs thickness",temperatureForTag(i));
xlabel {Thickness (nm)}
ylabel {L_\phi (m)}

%% L_SO vs D
getScatter(dif,lso,generalView,"L_{SO} vs D",temperatureForTag(i));
xlabel {D (m^2/s)}
ylabel {L_{SO} (m)}
...
%% Bso vs thickness
getScatter(thicknessForNum(1:number/2),bso,generalView,"B_{SO} vs thickness",temperatureForTag(i));
xlabel {Thickness (nm)}
ylabel {B_{SO} (T)}
